...
def load_config():
    ...
    if os.environ.__contains__("CONFIG"):
        try:
            ...
            print("CONFIG环境变量格式不正确，请检查Secret配置，请严格按照JSON格式：使用双引号包裹字段和值，逗号不能多也不能少")
            traceback.print_exc()
            exit(1)
    else:
        ...
        if os.path.exists(config_path):
            try:
                ...
                print(f"从本地 config.json 加载配置成功")
            except:
                print("config.json 格式不正确，请严格按照JSON格式：使用双引号包裹字段和值，逗号不能多也不能少")
                traceback.print_exc()
                exit(1)
        else:
            print("未配置CONFIG环境变量，且本地 config.json 文件不存在，无法执行")
            exit(1)
    return config, config_from_env


def load_aes_key():
    """从环境变量或本地 config.json 加载 AES_KEY"""
    if os.environ.__contains__("AES_KEY"):
        ...
        if aes_key is not None:
            ...
            if len(aes_key) == 16:
                ...
                print("AES_KEY环境变量未设置或者无效 无法使用加密保存功能")
    return None
...
if __name__ == "__main__":
    ...
    if aes_key is not None and len(aes_key) == 16:
        ...
        print("未设置或使用默认AES_KEY 无法使用加密保存功能")
    # 创建推送配置对象
